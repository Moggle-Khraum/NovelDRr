...
export const directFetchNovelMeta = async (url: string): Promise<NovelMeta> => {
  ...
  try {
    ...
    // Check for other similar sites (LightNovelWorld, NovelBin, AllNovelFull, LightNovelPub, NovelCool)
    const isSimilarSite = domainLower.includes('lightnovelworld') || 
                          domainLower.includes('novelbin') || 
    ...
    if (isReadNovelFull || isNovelFull || isSimilarSite) {
      ...
      const authorMatch = html.match(/<span[^>]*itemprop="author"[^>]*>.*?<meta[^>]*itemprop="name"[^>]*content="([^"]+)"/i) ||
                          html.match(/<div[^>]*class="info"[^>]*>.*?<h3[^>]*>Author:<\/h3>\s*<a[^>]*>([^<]+)<\/a>/i);
